...
def _parse_args():
    ...
    parser.add_argument(
        'n_masters',
        help='number of masters in the distributed system',
        type=int
    )
    parser.add_argument(
        'n_slaves',
        help='number of slaves per master in the distributed system',
        type=int
    )
    parser.add_argument(
        '--shared-memory-size',
...
class Worker(multiprocessing.Process):
    ...
    def run(self):
        ...
        env.numMasters = self.__configuration.n_masters
        env.slavesPerMaster = self.__configuration.n_slaves
        env.worker = self
        env.threadId = wid
        proxy = env.getProxyByThreadId(wid)

        if isinstance(proxy, MasterProxy):
            implementor = Master(env)
        else:
            implementor = Slave(env)
...
def main():
    ...
    configuration.n_workers = configuration.n_masters * (configuration.n_slaves + 1)
